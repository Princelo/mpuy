...
class UserController extends Controller implements WechatTokenGetterInterface
{
    ...
    public function listFollowingUsersAction(Request $request)
    {
        $em = $this->getDoctrine()->getEntityManager();
        $user = $this->getUser();
        $users = $em->getRepository('Acme\AccountBundle:User')->getFollowingUsersByUser($user, 0, 10);
    ...
    public function listFollowedUsersAction(Request $request)
    {
        $em = $this->getDoctrine()->getEntityManager();
        $user = $this->getUser();
        $users = $em->getRepository('Acme\AccountBundle:User')->getFollowedUsersByUser($user, 0, 10);
